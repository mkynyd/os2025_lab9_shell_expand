...
static void ShellCmd_Demo(const char *args)
{
    ...
    PRT_Printf("可以使用 'semstat' 查看信号量使用情况\n");
    PRT_Printf("演示任务优先级：High=5, Mid=10, Low=15\n");
    ...
    if (ret == OS_OK) {
        ...
        PRT_Printf("[Demo] 演示将在后台运行\n");
        PRT_Printf("[Demo] 提示：如果 Shell 暂时无响应，请等待演示任务结束\n");
    } else {
        PRT_Printf("[Demo] 创建演示任务失败: 0x%x\n", ret);
    }
}
